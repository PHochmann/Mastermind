...
{
    if (cell->parent != NULL)
        return get_h_align(default_h, cell->parent);
...
{
    if (cell->parent != NULL)
        return get_v_align(default_v, cell->parent);
...
static size_t get_total_width(const Table *table, const size_t *col_widths, struct Cell *cell)
{
    ...
    {
        if (i != 0 && table->border_left_counters[cell->x + i] > 0)
        {
            sum++;
        }
...
static size_t get_total_height(const Table *table, const size_t *row_heights, struct Cell *cell)
{
    ...
    {
        if (i != 0 && curr_row->border_above_counter > 0)
            sum++;
    ...
    while (curr_row != NULL)
    {
        for (size_t i = 0; i < table->num_cols; i++)
        {
            if (curr_row->cells[i].is_set && curr_row->cells[i].parent == NULL)
            {
                ...
                {
                    if (row_checked_for_hline == NULL || min == 0)
                        break;
                    if (row_checked_for_hline->border_above_counter > 0)
                        min--;
